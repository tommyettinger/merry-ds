...
public class MatchingTest {
	...
	public void testIntMapPut (){
		IntMap<Integer> merryMap = new IntMap<>();
	...
	public void testObjectMapPut (){
		ObjectMap<Integer, Integer> merryMap = new ObjectMap<>();
		HashMap<Integer, Integer> jdkMap = new HashMap<>();
		long stateA = 0L, stateB = 1L, temp;
		int merryRepeats = 0, jdkRepeats = 0;
		int item;
	...
	public void testObjectMapMix (){
		...
		{
			System.out.println("JDKMap is bigger: " + (jdkMap.size() - merryMap.size));
			ObjectMap.Keys<Integer> ks = merryMap.keys();
			while (ks.hasNext)
				jdkMap.remove(ks.next());
			System.out.println("M: " + merryMap.size);
			System.out.println("J: " + jdkMap.size());
